...
| `baseline.json` | Committed set of known, pre-existing violations per page |
| `tests/a11y.spec.ts` | Scans both apps, fails on new violations, writes reports |
...
1. `baseline.json` records the violation rule ids already present on each page.
...
3. If the apps genuinely fix a rule, remove that id from `baseline.json` so the suite
   keeps passing - and the recorded report documents the improvement.
